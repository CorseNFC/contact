  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="pt-24 pb-16 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p className="mt-3 text-slate-400 max-w-2xl">Une commande, plusieurs cartes personnalisées, une seule facture. Plus vous commandez, plus la remise augmente.</p>
          ...
          <div className="grid sm:grid-cols-4 gap-3 mt-8" data-testid="bulk-tiers">
            {[...pricing.tiers].reverse().map((t) => (
              <div key={t.min} className={`kt-card p-4 text-center transition ${qty >= t.min ? "border-amber-400/60 ring-1 ring-amber-500/30" : "opacity-70"}`} data-testid={`tier-${t.min}`}>
                ...
                <p className="font-display font-bold text-2xl">{t.min} <span className="text-sm font-normal text-slate-400">cartes</span></p>
          ...
          <div className="space-y-6">
            ...
            <div className="kt-card p-6">
              ...
              <div className="space-y-3">
                {cards.map((c, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-white/5 bg-slate-900/40 p-4" data-testid={`card-row-${i}`}>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs text-slate-500">Carte #{i + 1}</p>
                      {cards.length > 1 && (
                        <button onClick={() => removeCard(i)} className="text-slate-500 hover:text-red-400" data-testid={`remove-card-${i}`}>
                          <Trash2 size={13} />
                        </button>
                      )}
                    </div>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      ...
                      <select value={c.finish_id} onChange={(e) => setCard(i, { finish_id: e.target.value })} data-testid={`c${i}-finish`} className="px-3 py-2 rounded-md bg-slate-900/60 border border-white/10 text-slate-100 text-sm">
          ...
          <aside className="lg:sticky lg:top-24 h-fit">
            <div className="kt-card p-6" data-testid="bulk-summary">
              ...
              <div className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-400">Cartes</span><span className="font-medium">{qty}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Prix unitaire</span>
                  <span>{pct > 0 && <span className="text-slate-500 line-through mr-1 text-xs">{formatEUR(base)}</span>}<span className="font-medium">{formatEUR(unit)}</span></span>
                </div>
                {pct > 0 && (
              ...
              <p className="mt-3 text-[10px] text-slate-500 text-center">Facture PDF envoyée après paiement · Stripe sécurisé</p>
            </div>
          </aside>
        </div>
      </div>
...
const Field = ({ label, value, onChange, type = "text", testid }) => (
  <div>
    ...
    <Input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} data-testid={testid}
      className="bg-slate-900/60 border-white/10 focus:border-amber-400/60 text-slate-100" />
  </div>
);
const MiniField = ({ placeholder, value, onChange, type = "text", testid }) => (
  <Input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} data-testid={testid}
    className="bg-slate-900/60 border-white/10 focus:border-amber-400/60 text-slate-100 text-sm" />
);
