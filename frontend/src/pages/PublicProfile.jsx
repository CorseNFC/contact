...
export default function PublicProfile() {
  ...
  if (state.status === "loading") return <div className="min-h-screen grid place-items-center bg-slate-950" data-testid="profile-loading"><Loader2 className="animate-spin text-amber-400" /></div>;
  ...
  if (kind === "reviews") {
    return (
      <div className="min-h-screen bg-slate-950 grid place-items-center px-4" data-testid="reviews-redirect">
        <div className="max-w-sm text-center">
          ...
          <h1 className="mt-6 font-display text-2xl font-bold text-slate-100">Merci pour votre soutien !</h1>
          ...
          {p.reviews_url ? (
            ...
            <p className="mt-4 text-xs text-slate-500">Le propriétaire n'a pas encore configuré l'URL Google.</p>
          )}
        </div>
      </div>
    );
      ...
      <div className="min-h-screen w-full py-8 px-4" style={{ background: themeBg }} data-testid="pet-profile-page">
        <div className="max-w-md mx-auto">
          <div className="kt-card p-6 text-center">
            <div className="w-24 h-24 mx-auto rounded-full border-2 border-amber-400 overflow-hidden bg-slate-900 grid place-items-center">
            ...
            <p className="mt-6 text-[10px] text-slate-500">Propulsé par <span className="text-amber-400">KalliTag</span></p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen w-full" style={{ background: themeBg }} data-testid="public-profile-page">
      <div className="max-w-md mx-auto min-h-screen">
        ...
        <div className="px-5 pb-8 -mt-2">
          <button onClick={() => setLeadOpen((v) => !v)} className="w-full text-xs font-medium py-2.5 rounded-full border border-white/10 text-slate-300 hover:text-white hover:border-amber-400/40" data-testid="lead-toggle">
            {leadOpen ? "Annuler" : "Me laisser un message"}
          </button>
          {leadOpen && (
            <form onSubmit={submit} className="kt-card p-4 mt-3 space-y-3" data-testid="lead-form">
              <Input value={lead.name} onChange={(e) => setLead({ ...lead, name: e.target.value })} placeholder="Votre nom" required data-testid="lead-name" className="bg-slate-900/60 border-white/10 text-slate-100" />
              <Input value={lead.email} onChange={(e) => setLead({ ...lead, email: e.target.value })} placeholder="Email (optionnel)" type="email" data-testid="lead-email" className="bg-slate-900/60 border-white/10 text-slate-100" />
              <Input value={lead.phone} onChange={(e) => setLead({ ...lead, phone: e.target.value })} placeholder="Téléphone (optionnel)" data-testid="lead-phone" className="bg-slate-900/60 border-white/10 text-slate-100" />
              <textarea value={lead.message} onChange={(e) => setLead({ ...lead, message: e.target.value })} placeholder="Votre message" rows={3} className="w-full px-3 py-2 rounded-md bg-slate-900/60 border border-white/10 text-slate-100 text-sm" data-testid="lead-message" />
  ...
  const content = (
    <div className="flex items-start gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-sm text-slate-100 ${mono ? "font-mono text-xs" : ""}`}>{value}</p>
      </div>
    </div>
  );
